...
from pyFiles.dripEncoding import (create_drip_structure,
                                  ...
                                  load_drip_means, 
                                  drip_candidate_spectra_generator)
...
def make_drip_data_highres(args, data, 
    ...
    for s in spectra:
        ...
        if ion_dict:
            ...
            ####################### note: args.max_obs_mass updated in below function
            # max_obs_mass designates the maximum fragment mass, calculated as the max value against all 
            # fragment spectra
            make_master_parameters(args, mf, gf, mixf, cf,
                                   ion_dict, ions)
...
# ####################################################### 
    ...
    if args.num_jobs > 1:
        ...
        for data in candidate_binarydb_spectra_generator(args,
                                                         mods, nterm_mods, cterm_mods,
                                                         var_mods, nterm_var_mods, cterm_var_mods):
        # for data in drip_candidate_spectra_generator(args, var_mods, nterm_var_mods, cterm_var_mods):
